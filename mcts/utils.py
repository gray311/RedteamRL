...
from api.template import system_prompt, user_prompt, examples_with_responses, examples_wo_responses
...
def trajectory_to_response(trajectory):
    ...
    for i, step in enumerate(trajectory):
        response += f"Q{i+1}: {step['attacker_prompt']}\nA{i+1}: {extract_code(step['victim_response'].strip('<|im_end|>'))}\n"   
    return response.strip("\n")
  
def apply_chat_template(config, prompt, response, safetyinfo, n_turn, tokenizer, add_bos=False, simulate=False):
    goal = safetyinfo.goal
    code_type = safetyinfo.code_type
    if n_turn == 1:
        messages = [
            {
        ...
        messages.extend(examples_wo_responses)
        messages.append(
            {
                "role": "user",
                "content": "Goal:"+goal+ "\nPrompt: "+prompt.strip("\n")
        ...
        messages = [
            {
                "role": "system", "content": system_prompt + "\n\nPlease proceed with the user's dialogue with LLM and continue to fill in the rest of the questions and answers." ,
            },
        ]
        
        messages.extend(examples_with_responses)
        
        prompt = user_prompt.replace("<prompt>", goal)
